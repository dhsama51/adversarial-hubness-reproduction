...
OUT_DIR = os.path.expanduser("~/projects/hubness/results/openai_clip/ahd_input_universal_only")
os.makedirs(OUT_DIR, exist_ok=True)

gallery_emb = np.load(os.path.join(EMB_DIR, "image_embeddings.npy")).astype("float32")
base_n = gallery_emb.shape[0]

meta_files = sorted(glob.glob(os.path.join(RESULTS_DIR, "hub_universal_*_meta.json")))
...
print(f"Universal hub {len(hub_tags)}개 발견")
...
print(f"저장 완료: gallery {base_n} + universal hub {len(hub_tags)} = {n_total}개 "
      f"(비율 {len(hub_tags)/n_total*100:.2f}%)")
print(f"-> {OUT_DIR}")
os._exit(0)
