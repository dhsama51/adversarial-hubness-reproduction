...
def main():
    ...
    if args.mode == "universal":
        ...
        CLUSTER_CACHE_PATH = os.path.join(EMB_DIR, f"cluster_cache_k{args.n_clusters}.npz")
        ...
        if args.cluster_id is not None:
            ...
            assert candidates, "적당한 크기의 클러스터가 없습니다. --n_clusters 조정 필요"
        ...
        print(f"  예시 caption 3개: {[r['caption'] for r in qt_records[:3]]}")
    ...
    with torch.no_grad():
        inputs = processor(text=qt_captions, return_tensors="pt", padding=True, truncation=True).to(DEVICE)
        outputs = model.get_text_features(**inputs, return_dict=True)
        q_feats = outputs.pooler_output
    ...
    print(f"g_c 이미지 로드: {time.time() - t_fetch_start:.2f}초")
    ...
    print(f"\n최종 cos_sim(g_a, c_t) = {final_cos:.4f}")

    out_img = transforms.ToPILImage()(x_adv.detach().cpu().clamp(0, 1))
    img_path = os.path.join(RESULTS_DIR, f"hub_{tag}.png")
    ...
    meta = {
        ...
        "gc_index": gc_idx,
        "qt_query_ids": qt_query_ids,
        "n_qt": len(qt_query_ids),
        "eps": EPS,
        "steps": STEPS,
        "alpha": ALPHA,
    ...
    print(f"  임베딩     -> {RESULTS_DIR}/hub_{tag}_embedding.npy")
    print(f"  메타데이터 -> {RESULTS_DIR}/hub_{tag}_meta.json")
